...
public:
    ...
    void DecideSupportingParents(const std::vector<Obj3D::Ptr>& objects,
    ...
    void ComputeParentCandidates(const std::vector<Obj3D::Ptr>& objects,
            std::unordered_map<Obj3D::Ptr, std::vector<Obj3D::Ptr>>& candidates);
    void ComputeSupportingScores(const Obj3D::Ptr& child, const Obj3D::Ptr& parent,
            std::vector<std::pair<Eigen::Vector4f, float>>& scores);
    void UpdateObjectsViaSupportingRelations(std::vector<Obj3D::Ptr>& objects,
            std::unordered_map<Obj3D::Ptr,
                               std::unordered_map<Obj3D::Ptr, Eigen::Vector4f>>& parent_child_map,
    ...
    void ComputePlaneMatchingError(
            const std::vector<std::vector<Eigen::Vector4f>>& canonical_transformed_planes,
            const ObjCAD::Ptr& cad,
            std::unordered_map<int, float>& matching_errors,
    ...
    bool CheckSupportingAffordance(const Eigen::Vector4f& supporting_plane,
            const pcl::PointCloud<PointT>::Ptr& plane_cloud,
            const pcl::PointCloud<PointT>::Ptr& above_plane_cloud,
            const Obj3D::Ptr& child);
    bool ValidateCollisionFreeCriteria(const Obj3D::Ptr& object,
            ObjCADCandidate::Ptr& cad_candidate, const std::vector<Obj3D::Ptr>& objects);

    void ComputePotentialCollisionPairs(
            const std::unordered_map<Obj3D::Ptr,
                                     std::vector<ObjCADCandidate::Ptr>>& cad_selected_candidates,
            std::unordered_map<Obj3D::Ptr,
                               std::unordered_map<Obj3D::Ptr, float>>& possible_collision_map);

    void MatchCADToSegmentsFine(const std::vector<Obj3D::Ptr>& objects,
            std::unordered_map<Obj3D::Ptr,
                               std::vector<ObjCADCandidate::Ptr>>& cad_selected_candidates);

    float ComputeAlignmentError(const Obj3D::Ptr& object, ObjCADCandidate::Ptr& cad_candidate);

    void GlobalRegulation(const std::vector<Obj3D::Ptr>& objects,
            std::unordered_map<Obj3D::Ptr,
                               std::vector<ObjCADCandidate::Ptr>>& cad_selected_candidates,
            const std::unordered_map<Obj3D::Ptr,
                                     std::unordered_map<Obj3D::Ptr, float>>& possible_collision_map,
            std::unordered_map<Obj3D::Ptr, ObjCADCandidate::Ptr>& map_candidate);
    bool ValidateWallConstraint(const std::vector<Eigen::Vector4f>& wall_planes,
            const ObjCADCandidate::Ptr& cad_candidate);
    void ComputeCollision(const std::unordered_map<Obj3D::Ptr, ObjCADCandidate::Ptr>& map_candidate,
            const std::unordered_map<Obj3D::Ptr,
                                     std::unordered_map<Obj3D::Ptr, float>>& possible_collision_map,
            std::unordered_map<Obj3D::Ptr, std::vector<Obj3D::Ptr>>& collided_pairs,
            std::set<Obj3D::Ptr>& collided_objects);
    float RecomputeCollision(const Obj3D::Ptr& object,
            const ObjCADCandidate::Ptr& cad_candidate,
    ...
    void FillContactGraph(const std::vector<Obj3D::Ptr>& objects,
            const std::unordered_map<int, OBBox>& gt_objects,
            std::unordered_map<Obj3D::Ptr, ObjCADCandidate::Ptr>& map_candidate,
            pgm::ParseGraphMap::Ptr& contact_graph);

    Eigen::Matrix4f GetRefineTransform(const Eigen::Vector4f& param,
